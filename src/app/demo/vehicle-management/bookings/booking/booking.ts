...
export class Booking implements OnInit {
  ...
  loadAvailableVehicles(): void {
    ...
    // Try to load vehicles from backend API first
    this.bookingService.getAllVehicles().subscribe({
      next: (vehicles) => {
        console.log('Loaded vehicles from backend:', vehicles);
        this.availableVehicles = vehicles.filter(v => v.status === 'AVAILABLE');
        this.loadingVehicles = false;
      },
      error: (err) => {
        console.error('Error loading vehicles from backend, using fallback:', err);
        // Fallback: Load vehicles based on VehicleType enum
        this.loadFallbackVehicles();
      }
    });
  }

  private loadFallbackVehicles(): void {
    // Load vehicles based on VehicleType enum as fallback
    const vehicleTypes = Object.values(VehicleType);
    this.availableVehicles = [];

    // Create sample vehicles for each type
    vehicleTypes.forEach((type, index) => {
      // Create multiple vehicles of each type
      for (let i = 1; i <= 2; i++) {
        const vehicleId = `VH-${(index + 1).toString().padStart(2, '0')}${i}`;
        const vehicleNumber = this.generateVehicleNumber(type, i);
  ...
  private generateVehicleNumber(type: string, index: number): string {
    ...
    const prefix = prefixes[type] || 'VH';
    return `${prefix}-${Date.now().toString().slice(-4)}-${index}`;
  }

  private getModelForType(type: string): string {
    const models: { [key: string]: string } = {
      'LORI': 'Heavy Duty Lorry',
      'TRUCK': 'Tata Truck',
      'CAR': 'Sedan Car',
      'VAN': 'Cargo Van',
      'BUS': 'Passenger Bus'
    };
    return models[type] || 'Standard Vehicle';
  }

  private getCapacityForType(type: string): number {
  ...
  private getPriceForType(type: string): number {
  ...
  onSubmit() {
    ...
    // Find the selected vehicle from available vehicles
    let selectedVehicle = this.availableVehicles.find(v =>
      v.vehicleType === formValue.vehicleType && v.status === 'AVAILABLE'
    );

    // If no vehicle matches the type, assign any available vehicle
    if (!selectedVehicle) {
      selectedVehicle = this.availableVehicles.find(v => v.status === 'AVAILABLE');
    }
    ...
    const payload = {
      id: Date.now(), // Add required id
      bookingId: `BK-${Date.now().toString().slice(-6)}`, // Add required bookingId
      vehicle: selectedVehicle, // Use selected vehicle from VehicleType enum
      vehicleId: selectedVehicle.vehicleId, // Use actual vehicle ID
      vehicleNo: selectedVehicle.vehicleNumber, // Use actual vehicle number
      startedFrom: formValue.startedFrom,
      destination: formValue.destination,
      vehicleType: formValue.vehicleType,
      driverName: formValue.driverName || '',
      bookingHire: formValue.bookingHire || selectedVehicle.price || 0, // Use vehicle price as default
      bookingAdvance: formValue.bookingAdvance || 0,
      bookingBalance: formValue.bookingBalance || 0,
      bookingDate: formValue.bookingDate,
      bookingReceivedDate: formValue.bookingReceivedDate || null,
      detain: formValue.detain || '',
      podReceived: formValue.podReceived === 'true' || formValue.podReceived === true,
      podDocument: formValue.podDocument || '',
      lorryBalancePaidDate: formValue.lorryBalancePaidDate || null,
    ...
    this.bookingService.addBooking(payload).subscribe({
      ...
      error: (error) => {
        ...
        this.saveNewBookingToStorage(payload);
        this.router.navigate(['/booked-vehicles']);
      }
    });
  }
